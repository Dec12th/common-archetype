 ...
 */
public abstract class AbstractSpringStrategyManagerImpl<E extends BaseEnum, S extends Strategy<E>> extends AbstractStrategyManagerImpl<E, S> {
    /**
     * 设置列表
     */
    @Autowired(required = false)
    public void init(List<S> strategyList) {
        Map<E, S> strategyMap = new HashMap<>();
        if (CollectionUtils.isNotEmpty(strategyList)) {
            strategyList.forEach(
                    strategy -> strategyMap.put(strategy.getType(), strategy));
        }
        super.init(strategyMap);
    }
}
